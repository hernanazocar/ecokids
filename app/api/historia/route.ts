...
const dataPath = path.join(process.cwd(), 'data', 'historia.json');
...
export async function GET() {
  try {
    ...
    console.error('Error al leer historia:', error);
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Error al actualizar historia' }, { status: 500 });
  }
}
